...
当前站点支持中文 / 法文双语展示。页面静态文案、首页模块文案、场景与资源列表、制造图库标题以及产品展示字段的法文映射统一维护在 `src/i18n.ts`。
...
- 当前语言类型为 `CN | FR`，由 `src/i18n.ts` 中的 `useI18n()` 统一管理。
- Header 语言按钮会切换全站共享语言状态。
- 静态页面文案维护在 `messages` 映射中。
- 产品名称、描述、标签、参数和应用场景的法文展示维护在 `productTranslations` 映射中。
- 中文产品基础数据仍集中维护在 `src/data/products.ts`，法文内容不直接写入页面组件。
